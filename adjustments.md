...
2. If the response is a 422 status with an OperationOutcome:
   a. Check for the `subscription-adjusted` code in the `issue.details.coding`.
   b. If present:
      i. Start with the set of all initially requested `backport-filter-criteria`.
      ii. Remove any criteria that appear in the OperationOutcome's `original-criteria`.
      iii. Add any criteria that appear in the OperationOutcome's `adjusted-criteria`.
3. Create a new Subscription resource using the resulting set of criteria.
4. Submit a new POST request with the modified Subscription resource.
5. If the new request is successful (201 Created response), the subscription is established.
6. If another 422 response is received, repeat the process or abandon the attempt after a reasonable number of tries.
...
And the OperationOutcome response:
...
The resulting adjusted criteria would be:
...
Note that:
- The Encounter criteria was adjusted to remove the unsupported 'delete' trigger.
- The Observation criteria was split into two separate criteria for laboratory and vital-signs categories.
- The DiagnosticReport criteria was removed entirely as it appeared in `original-criteria` but had no corresponding `adjusted-criteria`.
